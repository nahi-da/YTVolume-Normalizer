browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === "ready") {
        ...
        const elem_input_volume_input = document.getElementById("input_volume");
        const elem_input_volume_val = document.getElementById("input_volume_val");
        const elem_output_volume_input = document.getElementById("output_volume");
        const elem_output_volume_val = document.getElementById("output_volume_val");
        ...
        elem_input_volume_input.value = localStorage.getItem("input_gain") * 100;
        elem_input_volume_val.textContent = localStorage.getItem("input_gain") * 100;
        elem_output_volume_input.value = localStorage.getItem("output_gain") * 100;
        elem_output_volume_val.textContent = localStorage.getItem("output_gain") * 100;
        ...
        elem_input_volume_val.textContent = elem_input_volume_input.value;
        elem_input_volume_input.addEventListener("input", (event) => {
            elem_input_volume_val.textContent = event.target.value;
            browser.tabs.query({active: true, currentWindow: true}).then((tabs) => {
                browser.tabs.sendMessage(tabs[0].id, {type: "set_input_gain", input_gain: event.target.value / 100});
            });
            localStorage.setItem("input_gain", event.target.value / 100);
        });
        elem_input_volume_input.addEventListener("wheel", function(event){
            if (event.deltaY < 0){
                elem_input_volume_input.valueAsNumber += 1;
            }else{
                elem_input_volume_input.value -= 1;
            }
            elem_input_volume_val.textContent = event.target.value;
            browser.tabs.query({active: true, currentWindow: true}).then((tabs) => {
                browser.tabs.sendMessage(tabs[0].id, {type: "set_input_gain", input_gain: event.target.value / 100});
        ...
        elem_output_volume_val.textContent = elem_output_volume_input.value;
        elem_output_volume_input.addEventListener("input", (event) => {
            elem_output_volume_val.textContent = event.target.value;
            browser.tabs.query({active: true, currentWindow: true}).then((tabs) => {
                browser.tabs.sendMessage(tabs[0].id, {type: "set_output_gain", output_gain: event.target.value / 100});
            });
            localStorage.setItem("output_gain", event.target.value / 100);
        });
        elem_output_volume_input.addEventListener("wheel", function(event){
            if (event.deltaY < 0){
                elem_output_volume_input.valueAsNumber += 1;
            }else{
                elem_output_volume_input.value -= 1;
            }
            elem_output_volume_val.textContent = event.target.value;
            browser.tabs.query({active: true, currentWindow: true}).then((tabs) => {
                browser.tabs.sendMessage(tabs[0].id, {type: "set_output_gain", output_gain: event.target.value / 100});
